...
the generated code performs AST manipulation in Haskell, making it harder to read than the comby rules of our approach. Asking the LLM to expand the set of rules,
 ...
 resemble bugs a developer might make (replacing 0 with 1 or using the wrong data type). However, GPT-4 also generates a surprisingly high number of equivalent mutants, generating 
 (essentially) equivalent strings for all logging output. One is unlikely to generate tests that can kill these mutants, unless checking the exact string contents of all 
 logging messages.
