...
\begin{lstlisting}[language=haskell,caption={Haskell Mutation Tool},label=lst:mutationTool]
...
\begin{lstlisting}[caption={GPT-4 Comby Haskell Rules},label=lst:haskellcomby]
head :[list] ==> last :[list]
tail :[list] ==> init :[list]
...
While LLMs are not able to generate fully working language specific mutation testing tools from scratch, they are capable of generating many interesting rules
 that are specific to a given language. For example, prompting GPT-4 to generate Haskell rules generates the set of rules seen in \Cref{haskellcomby}. The generated rules are 
