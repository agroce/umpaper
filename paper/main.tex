...
\usepackage{varwidth} % for achieving window that spans to the widest row
...
\lstdefinelanguage{haskell}{
  morekeywords={import,where,let,in,do,case,of,if,then,else,module,mutateAndTest,ParseFailed,ParseOk,mutateModule,mutateExp,main},
  sensitive=true,
  morecomment=[l]{--},
  morecomment=[n]{\{-}{-\}},
  morestring=[b]",
...
\acmConference[FSE '24]{Foundations of Software Engineering}{July 2024}{Brazil}
...
%\keywords{Software Testing, Mutants, Mutation Score, Equivalence}
...
%has been demonstrated to be possible, with a relatively small code
%base~\cite{regexpMut}.
...
\begin{lstlisting}[language=haskell,caption={\textbf{GPT-4 Generated Haskell Mutation Tool}},label=lst:mutationTool]
...
tool being unaware of Haskell's existence. Furthermore, like existing mutation testing tools for source code,
the generated code performs AST manipulation in Haskell, making it harder to read than the Comby rules of our approach. Asking the LLM to expand the set of rules
 ...
 such an approach would be a small subset of what our approach is
 ...
 logging messages. The error messages (potentially) could be checked,
 but the semantically equivalent meaning of these strings makes them
 poor mutants (which is why no mutation tools we are aware of
 implement such mutants).
...
While LLMs are not able to generate fully working language specific mutation testing tools from scratch, they are capable of generating many interesting rules
 ...
 both valid Comby syntax and language-specific. Specifically, replacing head with tail is a rule that would only apply to functional programming languages such as 
 Haskell.  This suggests that while extending other tools to include new rules
 ...
 language description of the desired rule, since GPT-4 already knows
...
language agnostic mutation testing tool~\cite{wodeltest}\footnote{An
  additional paper on this general topic is a 4 page tool paper for an initial
  prototype of our current tool, omitted for blinding.}. However, this tool uses
...
languages, and likely most not-so-commonly-used programming languages,
in very few lines of code.  Our
